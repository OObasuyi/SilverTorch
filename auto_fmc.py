...
class AugmentedWorker:
    def __init__(
        ...
        ftd_host="",
        domain="Global",
        ippp_location="ippp_test_file.csv",
        access_policy="test_acp",
        zbr_bypass: dict = None,
        rule_prepend_name="firewall",
        zone_of_last_resort="outside",
        same_cred=True,
    ):
        """
        ...
        for v in list(creds.values()):
            if not isinstance(v, (str, int, float)):
                raise ValueError(
                    f"Cred file has a value that is not allowed for this script. returned value of {type(v)}"
                )
    ...
    def rest_connection(self, reset=False):
        if reset:
            ...
            self.fmc = FMC(
                hostname=self.fmc_host,
                username=self.fmc_username,
    ...
    def fmc_net_port_info(self):
        ...
        def get_name_from_group_object(object_name: list, obj_type="net"):
            if isinstance(object_name, list):
                if obj_type == "net":
                    ...
                    return [
                        [str(i.get("name")), str(i.get("protocol")), str(i.get("port"))]
        ...
        for x in net_group_object:
            try:
                self.net_group_object.append(
                    tuple(
                        [
                            str(x["name"]),
                            get_name_from_group_object(x.get("objects")),
                            str(x["id"]),
                        ]
                    )
                )
                ...
                self.net_group_object.append(
                    tuple(
                        [
                            str(x["name"]),
                            get_name_from_group_object(x.get("literals")),
                            str(x["id"]),
                        ]
                    )
                )
        ...
        self.net_data = [
            tuple([str(x["name"]), str(x["value"]), str(x["id"])]) for x in net_objects
        ] + [
            tuple([str(x["name"]), str(x["value"]), str(x["id"])]) for x in host_objects
        ]
        self.port_data = [
            tuple(
                [
            ...
            for x in port_objects
        ]
        self.port_group_object = [
            tuple(
                [
                    str(x["name"]),
                    get_name_from_group_object(x.get("objects"), obj_type="port"),
                    str(x["id"]),
                ]
            )
            for x in port_group_object
        ]
    ...
    def retrieve_ippp(self):
        ...
        for origin in ["source", "destination"]:
            ...
            ippp[origin] = ippp[origin].apply(
                lambda x: (x.split("/")[0]).strip() if "/32" in x else x.strip()
            )
        ...
        if not na_protos.empty:
            self.logfmc.logger.warning(
                f"found protocols that cannot be used with this script\n Please enter them manually\n file location: {fpath}"
            )
        ...
        for col in ["service", "protocol"]:
            ...
            if col == "protocol":
                ippp[col] = ippp[col].apply(
                    lambda x: [
                        i.split()[0] for i in x.split("_") if i == "TCP" or i == "UDP"
                    ][0]
                )
        return ippp
    ...
    def create_fmc_object_names(self, keep_old_name=True):
        ...
        for i in self.ippp.index:
            ...
            if self.ippp["port_1"][i] in ["nan", "0", "65535", "any"] and self.ippp[
                "port_2"
            ][i] in ["nan", "0", "65535", "any"]:
                self.ippp["port_2"][i] = self.ippp["port_1"][i] = "any"
            elif self.ippp["port_2"][i] in ["nan", "0", "65535", "any"] and self.ippp[
                "port_1"
            ][i] in ["nan", "0", "65535", "any"]:
                self.ippp["port_2"][i] = self.ippp["port_1"][i] = self.ippp["port_2"][
                    i
                ] = "any"
        ...
        self.ippp["protocol"] = (
            self.ippp["protocol"].astype(str).apply(lambda x: x.strip()[:3])
        )
        self.ippp.drop(columns=["port_1", "port_2"], inplace=True)

        for type_ in tqdm(
            ["source", "destination", "port"],
            desc=f"creating new objects or checking if it exist.",
            total=3,
            colour="MAGENTA",
        ):
            ...
            if type_ != "port":
                for name_ip in self.net_data:
                    ...
                    if not self.ippp[self.ippp[type_] == name_ip[1]].empty:
                        self.ippp[f"fmc_name_{type_}"].loc[
                            self.ippp[type_] == name_ip[1]
                        ] = name_ip[0]
                        self.ippp[f"fmc_name_{type_}_install"].loc[
                            self.ippp[type_] == name_ip[1]
                        ] = False
            else:
                # check if port data already exist on fmc
                for port_info in self.port_data:
                    port_protco = self.ippp[
                        (self.ippp[type_] == port_info[2])
                        & (self.ippp["protocol"] == port_info[1])
                    ]
            ...
            if type_ in ["source", "destination"]:
                ...
                install_pd[f"fmc_name_{type_}"] = install_pd[type_].apply(
                    lambda net: f'{net.split("/")[0]}_{net.split("/")[1]}'
                    if "/" in net
                    else net
                )
                if not self.ippp[type_][
                    (self.ippp[f"fmc_name_{type_}_install"] == True)
                    & (self.ippp[type_] != "any")
                ].empty:
                    self.ippp[f"fmc_name_{type_}"] = self.ippp[type_][
                        (self.ippp[f"fmc_name_{type_}_install"] == True)
                        & (self.ippp[type_] != "any")
                    ].apply(
                        lambda net: f'{net.split("/")[0]}_{net.split("/")[1]}'
                ...
                net_list = list(
                    set([net for net in install_pd[f"fmc_name_{type_}"] if "_" in net])
                )
                net_list = [
                    {"name": net, "value": net.replace("_", "/")}
                    for net in net_list
                ...
                host_list = list(
                    set(
                        [
                            host
                            for host in install_pd[f"fmc_name_{type_}"]
                            if not "_" in host
                        ]
                    )
                )
                ...
                if not install_pd.empty:
                    ...
                    for i in gpl:
                        ...
                        i = i.iloc[0]
                        ipd = install_pd["service"][
                            (install_pd["port"] == i["port"])
                            & (install_pd["protocol"] == i["protocol"])
                        ]
                        spipd = self.ippp["service"][
                            (self.ippp["port"] == i["port"])
                            & (self.ippp["protocol"] == i["protocol"])
                        ]
                        install_pd[f"fmc_name_{type_}"][ipd.index.tolist()] = ipd.iloc[
                            0
                        ]
                        self.ippp[f"fmc_name_{type_}"][
                            spipd.index.tolist()
                        ] = spipd.iloc[0]

                    port_data = [po[0] for po in self.port_data]
                    install_pd[f"fmc_name_{type_}"] = install_pd[
                        f"fmc_name_{type_}"
                    ].apply(lambda port: port.replace(" ", "-"))
                    self.ippp[f"fmc_name_{type_}"] = self.ippp[
                        f"fmc_name_{type_}"
                    ].apply(lambda port: port.replace(" ", "-"))
                    ...
                    port_list = [
                        {
                            "name": port,
                            "protocol": install_pd["protocol"][
                                install_pd[f"fmc_name_{type_}"] == port
                            ].iloc[0],
                            "port": install_pd["port"][
                                install_pd[f"fmc_name_{type_}"] == port
                            ].iloc[0],
                        }
    ...
    def zone_to_ip_information(self):
        ...
        ftd_info = {
            ...
            "host": self.ftd_host,
            "username": self.ftd_username,
            "password": self.ftd_password,
        }
        ...
        nameif_output = [
            zone_name.strip().replace("nameif", "").strip()
            for zone_name in nameif_output.split("\n")
            if zone_name.startswith(" nameif")
        ]

        asp_output = [
            route_zone[3:].split()
            for route_zone in asp_output.split("\n")
        ...
        route_zone_info = route_zone_info[
            ~route_zone_info["ZONE"].str.contains("nlp.*tap")
        ]
        route_zone_info["CIDR"] = route_zone_info["MASK"].apply(
            lambda ip: IPv4Network(f"0.0.0.0/{ip}").prefixlen
            if search(ipv4_re, ip)
            else ip
        )
        route_zone_info = route_zone_info[
            route_zone_info["CIDR"].astype(str).str.isdigit()
        ]
        route_zone_info = route_zone_info[
            route_zone_info["ZONE"].astype(str) != "identity"
        ]
    ...
    def find_dup_policies(self, ruleset, acp_set):
        ...
        current_ruleset = self.fmc.policy.accesspolicy.accessrule.get(
            container_uuid=acp_set["id"]
        )
        changed_ruleset = []
        for i in current_ruleset:
            subset_rule = {}
            subset_rule["src_z"] = self.find_nested_group_objects(i.get("sourceZones"))
            subset_rule["dst_z"] = self.find_nested_group_objects(
                i.get("destinationZones")
            )
            subset_rule["source"] = self.find_nested_group_objects(
                i.get("sourceNetworks")
            )
            subset_rule["destination"] = self.find_nested_group_objects(
                i.get("destinationNetworks")
            )
            subset_rule["port"] = self.find_nested_group_objects(
                i.get("destinationPorts")
            )
        ...
        for ip in ["source", "destination"]:
            current_ruleset[f"real_{ip}"] = current_ruleset[ip].apply(
                lambda p: fdp_grouper(p, "ip")
            )
            ruleset[f"real_{ip}"] = ruleset[f"{ip}_network"].apply(
                lambda p: fdp_grouper(p, "ip")
            )

        self.logfmc.logger.warning(
            "getting real ports-protocols from named port objects"
        )
        current_ruleset["real_port"] = current_ruleset["port"].apply(
            lambda p: fdp_grouper(p, "port")
        )
        ...
        for col in ruleset.columns:
            ruleset[col] = ruleset[col].apply(
                lambda x: sorted(list(v for v in x))
                if isinstance(x, (tuple, list))
                else x
            )
        for col in current_ruleset.columns:
            current_ruleset[col] = current_ruleset[col].apply(
                lambda x: sorted(list(v for v in x))
                if isinstance(x, (tuple, list))
                else x
            )
        ...
        idx_collector = []
        for i in tqdm(
            current_ruleset.index,
            desc="Comparing old ruleset objects to new ones",
            total=len(current_ruleset.index),
            colour="yellow",
        ):
            for idx in ruleset.index:
                # bug fix for None(any) zone values slipping matching
                cur_src_z = (
                    current_ruleset["src_z"][i]
                    if current_ruleset["src_z"][i] != "None"
                    else "any"
                )
                cur_dst_z = (
                    current_ruleset["dst_z"][i]
                    if current_ruleset["dst_z"][i] != "None"
                    else "any"
                )
                ...
                # get nested objects
                cur_real_dst_ip = (
                    current_ruleset["real_destination"][i].split("|")
                    if "|" in current_ruleset["real_destination"][i]
                    else current_ruleset["real_destination"][i]
                )
                cur_real_src_ip = (
                    current_ruleset["real_source"][i].split("|")
                    if "|" in current_ruleset["real_source"][i]
                    else current_ruleset["real_source"][i]
                )
                cur_real_port_ip = (
                    current_ruleset["real_port"][i].split("|")
                    if "|" in current_ruleset["real_port"][i]
                    else current_ruleset["real_port"][i]
                )

                rs_real_dst_ip = (
                    ruleset["real_destination"][idx].split("|")
                    if "|" in ruleset["real_destination"][idx]
                    else ruleset["real_destination"][idx]
                )
                rs_real_src_ip = (
                    ruleset["real_source"][idx].split("|")
                    if "|" in ruleset["real_source"][idx]
                    else ruleset["real_source"][idx]
                )
                rs_real_port_ip = (
                    ruleset["real_port"][idx].split("|")
                    if "|" in ruleset["real_port"][idx]
                    else ruleset["real_port"][idx]
                )
                ...
                # zone
                if (
                    ruleset["source_zone"][idx] == cur_src_z
                    and ruleset["destination_zone"][idx] == cur_dst_z
                ):
                    quondam += 1
        ...
        try:
            ...
            self.logfmc.logger.warning(
                f"{'#' * 3}DROP {len(idx_collector)} DUP RULES{'#' * 3}"
            )
        except:
            # no dups to drop
            pass
    ...
    def _get_sn_match(self, type_, i):
        if self.ippp[type_][i] == "any":
            ...
            return {
                f"{type_}_zone": str(self.zbr_bypass[type_][i]),
                f"{type_}_network": self.ippp[f"fmc_name_{type_}"][i],
            }
        ...
        if "/" not in self.ippp[type_][i] or "/32" in self.ippp[type_][i]:
            for p in self.zone_ip_info.index:
                ...
                if ippp_subnet.subnet_of(ip_network(asp_subnet)):
                    return {
                        f"{type_}_zone": self.zone_ip_info["ZONE"][p],
                        f"{type_}_network": self.ippp[f"fmc_name_{type_}"][i],
                    }
        # if we need to find all zones a subnet might reside
            ...
            zone_group = tuple(
                list(
                    set(
                        [
                            self.zone_ip_info["ZONE"][p]
                            for p in self.zone_ip_info.index
                            if ip_network(self.zone_ip_info["ip_cidr"][p]).subnet_of(
                                ippp_subnet
                            )
                        ]
                    )
    ...
    def del_fmc_objects(self, type_, obj_type: str = None, where: str = None):
        ...
        if not isinstance(where, str):
            raise ValueError(
                f"where value is not type str. you passed an {type(where)} object"
            )
        permission_check(f"Are you sure you want to del ***{where}*** {type_} objects?")
        if type_ == "network":
            if obj_type == "net":
                del_list = (
                    [i[2] for i in self.net_data if where in i[0]]
                    if where != "all"
                    else self.net_data
                )
                for obj_id in tqdm(
                    del_list, total=len(del_list), desc=f"deleting {obj_type} objects"
                ):
                    try:
                        if "/" in obj_id[1]:
                        ...
                        self.logfmc.logger.error(
                            f"Cannot delete {obj_id} from set {obj_type} of {type_} \n received code: {error}"
                        )
            elif obj_type == "net_group":
                del_list = (
                    [i[2] for i in self.net_group_object if where in i[0]]
                    if where != "all"
                    else self.net_group_object
                )
                for obj_id in tqdm(
                    del_list, total=len(del_list), desc=f"deleting {obj_type} objects"
                ):
            ...
            if obj_type == "port":
                del_list = (
                    [i[3] for i in self.port_data if where in i[0]]
                    if where != "all"
                    else self.port_data
                )
                for obj_id in tqdm(
                ...
                del_list = (
                    [i[2] for i in self.port_group_object if where in i[0]]
                    if where != "all"
                    else self.port_group_object
                )
                for obj_id in tqdm(
                    del_list, total=len(del_list), desc=f"deleting {obj_type} objects"
                ):
            ...
            acp_rules = self.fmc.policy.accesspolicy.accessrule.get(
                container_uuid=acp_id["id"]
            )
            del_list = (
                [i["name"] for i in acp_rules if where in i["name"]]
                if where != "all"
                else acp_rules
            )
            for obj_id in tqdm(
                del_list, total=len(del_list), desc=f"deleting {where} rules"
            ):
                try:
                    self.fmc.policy.accesspolicy.accessrule.delete(
                        container_uuid=acp_id["id"], name=obj_id
                    )
                except Exception as error:
                    self.logfmc.logger.error(
                        f"Cannot delete {obj_id} from set {where} for rules \n received code: {error}"
                    )
        else:
            raise NotImplementedError(
                f"type_ not found please select rule, port, or network. you passed {type_}"
            )
    ...
    def create_acp_rule(self):
        ...
        if self.zbr_bypass is None:
            self.zone_ip_info["ip_cidr"] = (
                self.zone_ip_info["IP"].astype(str)
                + "/"
                + self.zone_ip_info["CIDR"].astype(str)
            )
            # sort df by subnet size to find the closet match first
            self.zone_ip_info.sort_values(
                by="ip_cidr",
                key=lambda x: x.apply(lambda y: ip_network(y)),
                ascending=False,
                inplace=True,
            )
        else:
            ...
            if not isinstance(self.zbr_bypass, dict):
                raise TypeError(
                    f"zbr_bypass is a {type(self.zbr_bypass)} object not dict"
                )
        ...
        for i in self.ippp.index:
            ...
            rule_flow.update(
                {
                    "port": self.ippp["fmc_name_port"][i]
                    if self.ippp["port"][i] != "any"
                    else "any"
                }
            )
        ...
        try:
            for col in ruleset.columns:
                ruleset[col] = ruleset[col].apply(
                    lambda x: tuple(v for v in x) if isinstance(x, list) else x
                )
        ...
        ruleset_holder = []
        dup_holder = []
        for grouped_df, type_net in zip(
            [case1, case2, case3], ["source", "destination", "zone"]
        ):
            group_listing = grouped_df.size()[
                grouped_df.size() > 1
            ].index.values.tolist()
            for gl in group_listing:
                ...
                if type_net == "source" or type_net == "destination":
                    ...
                    agg_dst_net = sorted(
                        list(set(group["destination_network"].tolist()))
                    )
                    ...
                    group["source_network"] = (
                        agg_src_net if len(agg_src_net) > 1 else agg_src_net[0]
                    )
                    group["destination_network"] = (
                        agg_dst_net if len(agg_dst_net) > 1 else agg_dst_net[0]
                    )
                # dup policy check
                dup_seen = False
        ...
        for col in ruleset.columns:
            ruleset[col] = ruleset[col].apply(
                lambda x: list(v for v in x) if isinstance(x, tuple) else x
            )
        ...
        dt_now = datetime.now().replace(microsecond=0).strftime("%Y%m%d%H%M%S")
        ruleset_loc = create_file_path(
            "predeploy_rules",
            f"fmc_ruleset_preload_configs_{dt_now}.csv",
        )
        ...
        # get all zone info
        all_zones = {
            fix_object(i)[0]["name"]: fix_object(i)[0]
            for i in self.fmc.object.securityzone.get()
        }
        # create a bulk policy push operation
        charity_policy = []
        for i in tqdm(
            ruleset.index,
            desc="Loading bulk rule collection artifacts",
            total=len(ruleset.index),
            colour="green",
        ):
                        ...
                        while not matched:
                            if "destination" in k or "source" in k:
                                ...
                                if not matched:
                                    ...
                                    create_group_obj = {
                                        "objects": [
                                            {"type": name_ip_id[1], "id": name_ip_id[2]}
                                            for ip in v
                                            for name_ip_id in self.net_data
                                            if ip == name_ip_id[0]
                                        ],
                                        "name": f"{self.rule_prepend_name}_net_group_{randint(1, 100)}",
                                    }
                                    try:
                                        if len(create_group_obj["objects"]) > 1:
                                            response = (
                                                self.fmc.object.networkgroup.create(
                                                    create_group_obj
                                                )
                                            )
                                            if "already exists" not in str(response):
                                                self._creation_check(
                                                    response,
                                                    create_group_obj["name"],
                                                    output=False,
                                                )
                                        matched = True
                                ...
                                for port_lists in self.port_group_object:
                                    port_list_name = [
                                        p_name[0] for p_name in port_lists[1]
                                    ]
                                ...
                                if not matched:
                                    create_group_obj = {
                                        "objects": [
                                            ...
                                            for port in v
                                            for name_port_id in self.port_data
                                            if port == name_port_id[0]
                                        ],
                                        "name": f"{self.rule_prepend_name}_port_group_{randint(1, 100)}",
                                    }
                                    try:
                                        if len(create_group_obj["objects"]) > 1:
                                            response = (
                                                self.fmc.object.portobjectgroup.create(
                                                    create_group_obj
                                                )
                                            )
                                            if "already exists" not in str(response):
            ...
            rule_form[
                "name"
            ] = f"{self.rule_prepend_name}_{rule['comment']}_{randint(1, 1000000)}"
            ...
            for srcdest_net in ["source", "destination"]:
                if "any" != rule[f"{srcdest_net}_network"]:
                    if "group" in rule[f"{srcdest_net}_network"]:
                        ...
                        self.fmc_net_port_info()
                        rule_form[f"{srcdest_net}Networks"] = {
                            "objects": fix_object(
                                self.fmc.object.networkgroup.get(
                                    name=rule[f"{srcdest_net}_network"]
                                )
                            )
                        }
                    else:
                            ...
                            "objects": [
                                {
                                    "name": i[0],
                                    "id": i[2],
                                    "type": "Host" if "/" not in i[1] else "Network",
                                }
                                for i in self.net_data
                                if i[0] == rule[f"{srcdest_net}_network"]
                            ]
                        }
            ...
            for srcdest_z in ["source", "destination"]:
                if all(
                    [
                        "any" != rule[f"{srcdest_z}_zone"],
                        "any" not in rule[f"{srcdest_z}_zone"],
                    ]
                ):
                    if isinstance(rule[f"{srcdest_z}_zone"], list):
                        ...
                        rule_form[f"{srcdest_z}Zones"] = {
                            "objects": [all_zones[rule[f"{srcdest_z}_zone"]]]
                        }
            ...
            if "any" != rule["port"]:
                if "group" in rule["port"]:
                    # update npi if we created a grouped policy
                    self.fmc_net_port_info()
                    rule_form["destinationPorts"] = {
                        "objects": fix_object(
                            self.fmc.object.portobjectgroup.get(name=rule["port"])
                        )
                    }
                else:
                    rule_form["destinationPorts"] = {
                        "objects": [
                            {"name": i[0], "id": i[3], "type": i[4]}
                            for i in self.port_data
                            if i[0] == rule["port"]
                        ]
                    }
        ...
        try:
            res = self.fmc.policy.accesspolicy.accessrule.create(
                data=charity_policy,
                container_uuid=acp_set["id"],
...
if __name__ == "__main__":
    augWork = AugmentedWorker(
        ippp_location="gfrs.csv",
        access_policy="test12",
        ftd_host="10.11.6.191",
        fmc_host="10.11.6.60",
        rule_prepend_name="test_st_beta_1",
